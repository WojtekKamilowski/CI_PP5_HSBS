...
class StripeWH_Handler:
    ...
    def _send_confirmation_email(self, order):
        """
        Sends the user an email with order confirmation
        """
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        # Updates user profile detials when save_info checked
        profile = None
        ...
        if order_exists:
            ...
            return HttpResponse(
                content=f'Webhook received: {event["type"]} |ver ord al in db',
                status=200)
        else:
            order = None
            try:
        ...
        self._send_confirmation_email(order)
        return HttpResponse(
            content=f'Webhook received: {event["type"]} |Crtd ordr in webhook',
            status=200)
